...
class FbPage extends Model
{
    ...
    public static function get_feed_infor($fb_id = '') {
        if (! $fb_id)
            return false;

        $url = self::$api_url . $fb_id . "?access_token=" . self::get_fb_access_token();
        $obj = json_decode(HelperNews::curl($url));

        $link = '';
        if (! empty($obj->link)) {
            //$link = HelperNews::genBitly($obj->link);
            $link = $obj->link;
        }
    ...
    public static function search_fb_pages($query = '')
    {
        if (empty($query))
            return false;
        ...
        $result = array();
        if (!empty($obj->data) && count($obj->data)) {
            $request = array();
            foreach ($obj->data as $item) {
                $batch = new \stdClass;
                $batch->method = 'GET';
                $batch->relative_url = $item->id;
                $request[] = $batch;
            }
            $url = self::$api_url . '?batch=' . urlencode(json_encode($request)) . '&access_token=' . self::get_fb_access_token() . '&method=post';
            ...
            if (!empty($objs)) {
                foreach ($objs as $i => $obj) {
                    if ($obj->body) {
                        ...
                        $page->id = $body->id;
                        $page->name = $body->name;
                        $page->likes = floatval($body->likes);
                        $page->talking_about = $body->talking_about_count;
                        $page->thumb = "http://graph.facebook.com/{$body->id}/picture?type=small";
                        $page->page_link = "https://www.facebook.com/" . $body->id;
                        $page->app_page_link = "fb://page/" . $body->id;
                        $result[] = $page;
                    }
                }
            }
        ...
        usort($result, function ($a, $b) {
            return (float) $a->likes < (float) $b->likes;
        });
    ...
    public static function get_fb_page_infor($page_link = '', $category_id = 0, $status = 0)
    {
        ...
        if (!empty($obj->name)) {
            ...
            $record->name = (isset($obj->name)) ? $obj->name : '';
            $record->fb_id = (isset($obj->id)) ? $obj->id : '';
            $record->like_count = (isset($obj->likes)) ? $obj->likes : 0;
