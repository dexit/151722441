...
class DLN_Blocks {
	...
	function __construct() {
		...
		add_shortcode( 'dln_product_item', array( $this, 'shortcode_dln_product_item' ) );
	}
	...
	public function shortcode_dln_product_item() {
		return self::get_block( 'product-item' );
	}
	...
	public static function load_block_class( $form_name ) {
		...
		$block_file  = DLN_CLF_PLUGIN_DIR . '/dln-block/includes/blocks/block-' . $form_name . '.php';
	...
	public static function block_locate_template( $template_name, $template_path = '', $default_path = '' ) {
		...
		if ( ! $default_path )
			$default_path = DLN_CLF_PLUGIN_DIR . '/dln-block/includes/templates/';
