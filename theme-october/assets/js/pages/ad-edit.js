(function ($) {
    ...
    AdEdit.prototype.initEvents = function () {
        ...
        $('#dln_name').on({
            change: function (e) {
                ...
                if (count >= 0) {
                    $('#dln_name_count').val(count);
                } else {
                    $('#dln_name_count').val(0);
                }
            },
            blur: function (e) {
        ...
        $('#dln_desc').on({
            change: function (e) {
                ...
                if (count >= 0) {
                    $('#dln_desc_count').val(count);
                } else {
                    $('#dln_desc_count').val(0);
                }
            },
            blur: function (e) {
                self.checkFormDesc(true);
        ...
        $('#dln_save_photo_order').on('click', function (e) {
            ...
            if (photo_ids.length) {
                ...
                $.ajax({
                    type: 'POST',
                    url: window.root_api_url + '/photo/order',
                    data: {
                        photo_ids: photo_ids
                    },
    ...
    AdEdit.prototype.showChecked = function (selector) {
        ...
        $('div[data-relate="' + selector + '"] .label-success').removeClass('hide');
        $('div[data-relate="' + selector + '"] .label-default').addClass('hide');
    };

    AdEdit.prototype.hideChecked = function (selector) {
        $('div[data-relate="' + selector + '"]').removeClass('completed');
        $('div[data-relate="' + selector + '"] .label-success').addClass('hide');
        $('div[data-relate="' + selector + '"] .label-default').removeClass('hide');
    };
        ...
        if (form.valid()) {
            ...
            if (calc) {
                ...
                if (self.$ad_id) {
                    ...
                    $.ajax({
                        type: 'PUT',
                        url: window.root_api_url + '/ad/' + self.$ad_id,
                        data: form.serialize(),
                        success: function (res) {
                            if (res.status == 'success') {
                                self.hideStatus();
    ...
    AdEdit.prototype.checkFormRoom = function (calc) {
        var self = this;

        var form = $('#ad_room form');
