 ...
 */
function spClass($class_name, $args = null, $sdir = null, $force_inst = FALSE, $inst_index=NULL){
	if($inst_index==null){
		$inst_index = "default";
	}
	...
	if(TRUE != $force_inst)if(isset($GLOBALS['G_SP']["inst_class"][$class_name][$inst_index]))return $GLOBALS['G_SP']["inst_class"][$class_name][$inst_index];
...
function spError($msg, $output = TRUE, $stop = TRUE){
	...
	if(TRUE == $GLOBALS['G_SP']['optimizer']['gzip']){
		...
		$__speedy_compression_level = $GLOBALS['G_SP']['optimizer']['gzip_compression_level']?$GLOBALS['G_SP']['optimizer']['gzip_compression_level']:9;
