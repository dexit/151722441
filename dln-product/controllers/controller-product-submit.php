...
class DLN_Controller_Product_Submit {
		...
		self::$fields = apply_filters(
			...
			array(
				'basic' => array(
					array(
						'id'    => 'product_img',
						'label' => __( 'Photo', DLN_PRO ),
						'type'  => 'select-photo',
					),
					array(
						'id'          => 'product_title',
						'label'       => __( 'Title', DLN_PRO ),
						'type'        => 'text',
						'value'       => '',
						'placeholder' => __( 'Product Title', DLN_PRO ),
						'required'    => true,
					),
					array(
						'id'          => 'product_category',
					...
					array(
						'id'          => 'product_price',
						'label'       => __( 'Price', DLN_PRO ),
						'type'        => 'text-append',
						'value'       => '',
						'placeholder' => __( '', DLN_PRO ),
						'required'    => true,
						'append'      => __( '.000 vnÄ‘', DLN_PRO ),
						'input_type'  => 'number',
					),
					array(
						'id'    => 'product_desc',
						'label' => __( 'Description', DLN_PRO ),
						'type'  => 'textcomplete',
	...
	private static function get_category_options() {
		...
		if ( empty( $categories ) || ! empty( $categories->errors ) ) {
			$result = new WP_Error( '500', __( 'Product Categories Not Found!', DLN_PRO ) );
			return $result;
		}
		$options = array();
		if ( ! empty( $categories ) ) {
	...
	private static function load_frontend_assets() {
		wp_enqueue_script( 'dln-block-product-submit-js', DLN_PRO_PLUGIN_URL . '/assets/dln-abe/js/block-product-submit.js', array( 'jquery' ), '1.0.0', true );
